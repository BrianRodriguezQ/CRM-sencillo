...
import { Phone, Store, Power, PowerOff, Search, Activity, CalendarDays } from 'lucide-react'
import { useSellersList, useToggleUserActive } from '../../hooks/queries/useTeam'
import { useOrdersList } from '../../hooks/queries/useOrders'
import { ORDER_STATUS_LABELS } from '../../lib/order-status'
import type { Seller } from '../../hooks/queries/useTeam'
...
export function SellersPage() {
  ...
  const { data: list, isLoading } = useSellersList({
    page,
    perPage,
  ...
  const handleStatusFilter = (value: 'activo' | 'inactivo' | '') => {
  ...
  return (
    <div className="mx-auto max-w-6xl">
        ...
        <div className="p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <select
              ...
              value={statusFilter}
              onChange={(e) => handleStatusFilter(e.target.value as 'activo' | 'inactivo' | '')}
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {sellers.map((s) => (
              <SellerCard key={s.id} seller={s} onViewActivity={() => setActiveSeller(s)} />
            ))}
          </div>
      ...
      {activeSeller && (
        <SellerActivityModal seller={activeSeller} onClose={() => setActiveSeller(null)} />
      )}
    </div>
  )
}

function SellerCard({ seller, onViewActivity }: { seller: Seller; onViewActivity: () => void }) {
  ...
  return (
    <Card className="p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 flex items-center gap-1 text-xs text-gray-500">
              <Store className="h-3 w-3" /> {seller.orderCount} órdenes levantadas
            </p>
          </div>
        </div>
        <span
          className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${
            seller.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'
          }`}
        >
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              seller.isActive ? 'bg-green-500' : 'bg-gray-400'
            }`}
          />
      ...
      <div className="mt-3 flex items-center justify-end gap-1">
        ...
        <Button
          type="button"
          variant={seller.isActive ? 'danger' : 'secondary'}
          size="sm"
          loading={isPending}
          onClick={() => toggleActive(seller.id)}
        >
          {seller.isActive ? (
...
function SellerActivityModal({ seller, onClose }: { seller: Seller; onClose: () => void }) {
  ...
  const stats = useMemo(() => {
    ...
    const totalAmount = orders.reduce((acc, o) => acc + (Number(o.amount) || 0), 0)
    return { delivered: delivered.length, totalAmount }
  }, [orders])

  return (
    <Modal isOpen onClose={onClose} title={`Actividad de ${seller.name}`} size="lg" zIndex={90}>
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        <div className="rounded-lg border border-spi-border bg-gray-50 dark:bg-white/5 p-3">
          <p className="text-xs text-gray-500">Órdenes levantadas</p>
        ...
        <div className="rounded-lg border border-spi-border bg-gray-50 dark:bg-white/5 p-3">
          ...
          <p className="mt-0.5 text-xl font-bold text-spi-green">{formatMoney(stats.totalAmount)}</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description={`No hay órdenes levantadas por ${seller.name} en este período.`}
        />
      ) : (
